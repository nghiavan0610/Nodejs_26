...
class MeController {
  ...
  //       paranoid: false,
  //     }),
  //     Like_res.count({ where: { group: 'user_id' } }),
  //   ])
  //     .then(([users, deletedCount, likedCount]) =>
  ...
  storedUsers(req, res, next) {
    Promise.all([
      sequelize.query('select User.user_id, User.full_name as user_name, User.email, User.createdAt, count(Like_res.res_id) as new from User left join Like_res on User.user_id = Like_res.user_id group by User.user_id' , {
